...
def main():
    print("=== Launching Local Backend Server for Pilot-Launch v10 Benchmark ===")
    ...
    log_file = open("backend_benchmark_v10.log", "w", encoding="utf-8")
    ...
    print("=== Running Complete 45-Question v10 Benchmark Harness ===")

    env = os.environ.copy()
    env["JUSTOR_BACKEND_URL"] = "http://127.0.0.1:10001"

    out_file = "brenchmark/benchmark_results_pilot_launch_v10.csv"
    harness_cmd = [
        py_cmd,
    ...
    if os.path.exists(out_file):
        ...
        print("\n================================================")
        print("     JUSTOR AI PILOT-LAUNCH v10 BENCHMARK       ")
        print("================================================")
        total = len(df)
        http500 = df['system_answer'].str.contains('500|Internal Server Error|HTTPError', case=False, na=False).sum()
        http503 = df['system_answer'].str.contains('503|service busy|service unavailable', case=False, na=False).sum()
        not_found = df['system_answer'].str.contains('not locate|not in my verified|error|503', case=False, na=False).sum()
        ...
        clean_mask = ~(
            df['system_answer'].str.contains('500|Internal Server Error|HTTPError|not locate|not in my verified|503', case=False, na=False) |
        ...
        print(f"Fabricated Citations:        {fab_cit} (0% Hallucination)")
        print(f"Act Mismatches:              {act_mis}")
        print(f"Jurisdiction Leakage:        {jur_leak} (Strict Bangladesh Law)")
        print("------------------------------------------------")
        print(f"v10 PILOT-LAUNCH PASS RATE:  {pass_rate:.1f}% ({clean_count}/{total})")
        print("================================================\n")
