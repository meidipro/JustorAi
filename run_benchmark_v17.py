...
def main():
    print("=== Launching Local Backend Server for 195-Question Benchmark ===")
    ...
    log_file = open("backend_benchmark_195.log", "w", encoding="utf-8")
    ...
    print("=== Running Complete 195-Question Benchmark Harness ===")

    env = os.environ.copy()
    env["JUSTOR_BACKEND_URL"] = "http://127.0.0.1:10001"

    out_file = "brenchmark/benchmark_results_195_v17.csv"
    harness_cmd = [
        py_cmd,
        "brenchmark/benchmark harness.py",
        "--input", "brenchmark/justor_benchmark_195.csv",
        "--output", out_file,
        "--skip-ragas"
    ]
    ...
    if os.path.exists(out_file):
        ...
        print("\n================================================")
        print("     JUSTOR AI 195-QUESTION BENCHMARK RESULTS   ")
        print("================================================")
        total = len(df)
        ...
        print(f"DLR Case Law Cited:          {dlr_cites} questions")
        ...
        print("------------------------------------------------")
        print(f"195-QUESTION PASS RATE:      {pass_rate:.1f}% ({clean_count}/{total})")
        print("================================================\n")
